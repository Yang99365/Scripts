...
public class BoardCreator : MonoBehaviour 
{
	...
	public void Save ()
	{
		...
		board.tileTypes = new List<Tile.TileType>( tiles.Count );
		
		foreach (Tile t in tiles.Values)
		{
			board.tiles.Add( new Vector3(t.pos.x, t.height, t.pos.y) );
			board.tileTypes.Add(t.type);
		}
		
		string fileName = string.Format("Assets/Resources/Levels/{1}.asset", filePath, name);
		AssetDatabase.CreateAsset(board, fileName);
		/*
		// 타일의 데이터를 저장(위치와 타일의 높이, 타일의 타입을 저장)
		string filePath = Application.dataPath + "/Resources/Levels";
		if (!Directory.Exists(filePath))
			CreateSaveDirectory ();
    ...
    if (levelData == null)
		...
		{
			if (i >= levelData.tileTypes.Count)
				break;
			
			Vector3 v = levelData.tiles[i];
			Tile.TileType type = levelData.tileTypes[i];
			Tile t = Create(type);
			t.Load(v);
			t.Materialize((int)type);
			tiles.Add(t.pos, t);
		}
		/*
	...
	Tile Create(Tile.TileType type)
	{
		Tile t = Create();
		t.type = type;
		return t;
	...
	{
		if (tiles.ContainsKey(p)) // 이미 타일이 있다면 그 타일을 반환
			return tiles[p];
		
		Tile t = Create();
		t.Load(p, 0);
		t.Materialize(materialIndex);
		tiles.Add(p, t);
