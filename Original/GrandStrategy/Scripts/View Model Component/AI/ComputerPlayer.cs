...
public class ComputerPlayer : MonoBehaviour 
{
	#region Fields
	BattleController bc;
	GeneralUnit actor { get { return bc.turn.actor; }}
	Alliance alliance { get { return actor.GetComponent<Alliance>(); }}
	GeneralUnit nearestFoe;
	#endregion
	...
	#region Public
	public PlanOfAttack Evaluate () // 어떤 능력을 사용할지 조건부로 결정합니다.
	{
	...
	bool IsPositionIndependent (PlanOfAttack poa) // 위치는 중요한가?
	{
		AbilityRange range = poa.ability.GetComponent<AbilityRange>();
		return range.positionOriented == false;
	}
	
	bool IsDirectionIndependent (PlanOfAttack poa) // 방향이 중요하지 않은가?
	{
		AbilityRange range = poa.ability.GetComponent<AbilityRange>();
		return !range.directionOriented;
	}
	
	void PlanPositionIndependent (PlanOfAttack poa) // 랜덤으로 이동
	{
	...
	void PlanDirectionIndependent (PlanOfAttack poa) // 위치는 중요하지만 방향은 중요하지 않음
	{
	...
	//보드 위의 유닛 위치와 선택한 능력을 사용하는 동안 유닛이 향하는 방향에 따라 달라집니다.
	...
	bool IsAbilityTargetMatch (PlanOfAttack poa, Tile tile) // 맞출 대상인지 확인
	{
		bool isMatch = false;
	...
	List<Tile> GetMoveOptions () // 현재 행위자가 닿을수있는 타일 목록 반환
	{
		return actor.GetComponent<Movement>().GetTilesInRange(bc.board);
	}
	
	void RateFireLocation (PlanOfAttack poa, AttackOption option) // 공격 위치 평가
	{
	...
	void PickBestOption (PlanOfAttack poa, List<AttackOption> list) // 최고의 공격 옵션 선택
	{
		int bestScore = 1;
		...
		{
			poa.ability = null; // 능력을 수행하지 않는다는 표시로 명확한 능력
			return;
		}
	...
	void FindNearestFoe () // 가장 가까운 적 탐색
	{
		nearestFoe = null;
		bc.board.Search(actor.tile, delegate(Tile arg1, Tile arg2) {
			if (nearestFoe == null && arg2.content != null)
			{
				...
				{
					GeneralUnit unit = other.GetComponent<GeneralUnit>();
	...
	void MoveTowardOpponent (PlanOfAttack poa) // 가장 가까운 적으로 이동
	{
		List<Tile> moveOptions = GetMoveOptions();
		FindNearestFoe();
		if (nearestFoe != null)
	...
	public Directions DetermineEndFacingDirection () // 공격후 보는 방향 결정 (대체로 적이 있는 방향으로)
	{
