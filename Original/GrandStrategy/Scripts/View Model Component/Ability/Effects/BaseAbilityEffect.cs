...
public abstract class BaseAbilityEffect : MonoBehaviour
{
    ...
    protected virtual int GetStat (GeneralUnit attacker, GeneralUnit target, string notification, int startValue)
	{
		var mods = new List<ValueModifier>();
		var info = new Info<GeneralUnit, GeneralUnit, List<ValueModifier>>(attacker, target, mods);
